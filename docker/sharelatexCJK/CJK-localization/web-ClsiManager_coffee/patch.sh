...
# sed operation
sed -i -e "s/VALID_COMPILERS: \[\"pdflatex\", \"latex\", \"xelatex\", \"lualatex\"\]/HOGEHOGE1/" ${CURDIR}/${FILENAME}.patched ;  ${DEBUG}
grep "HOGEHOGE1" ${CURDIR}/${FILENAME}.patched &>/dev/null ; if [ "$?" = "1" ]; then exit 11 ; fi
sed -i -e "s/HOGEHOGE1/VALID_COMPILERS: \[\"pdflatex\", \"latex\", \"xelatex\", \"lualatex\", \"ctex\", \"platex\", \"hlatex\"\]/" ${CURDIR}/${FILENAME}.patched ;  ${DEBUG}
